...
const  queryDataPDF= async(id)=>{
          ...
          const res=await axios.get("https://aipcattle.herokuapp.com/abdominal/history/" +props.UID+"/"+id)
     ...
     console.log(res.data);
         ...
         dataSet.push([arrData[5],arrData[11],arrData[8],arrData[7]])
          })
...
const PDF=(data,profile,databrand)=>{
  ...
  doc.autoTable({
    startY: finalY + 50,
    head: [['วันที่', 'ผลการตรวจ','ผู้ตรวจ','หมายเหตุ']],
    columnStyles: {
      0: {cellWidth:30},
      1: {cellWidth: 45},
      2: {cellWidth: 45},
