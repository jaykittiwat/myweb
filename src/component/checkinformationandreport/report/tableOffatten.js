...
const PDF=(data,profile,databrand)=>{
  ...
  doc.text('ใบประวัติการบำรุง', 85, finalY + 15)
  doc.setFontSize(18)
  ...
  doc.autoTable({
    startY: finalY + 50,
    head: [['วันที่', 'เวลา','ผู้ปฏิบัติ', 'โปรแกรมบำรุง']],
    columnStyles: {
      0: {cellWidth:25},
      1: {cellWidth: 45},
      2: {cellWidth: 45},
...
const queryDataExcel=async(id,index)=>{
   ...
   res.data.map(i=>{
    const newSet=[
      ...
      {
        value: mapDrug(i.detail),style: {border: borders,alignment: {wrapText: true, horizontal: 'left', vertical: 'top'}}
...
const multiDataSet=[
  ...
  { 
    ...
    columns: [
      ...
      {title: "เวลา", width: {wpx:100}, style: { border: borders, font: { bold: true }},},//char width 
      {title: "ผู้ปฏิบัติ", width: {wpx:100}, style: { border: borders, font: { bold: true }},},
      {title: "โปรแกรมบำรุง", width: {wpx:150}, style: { border: borders, font: { bold: true }},},
  ],
  data:data
  }
]
