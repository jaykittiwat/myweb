...
const  queryDataPDF= async(id)=>{
          ...
          const res=await axios.get("https://aipcattle.herokuapp.com/delivery/history/" +props.UID+"/"+id)
         ...
         dataSet.push([arrData[2],arrData[0],arrData[7],arrData[6],arrData[4],arrData[3]])
          })
...
const PDF=(data,profile,databrand)=>{
  ...
  doc.text('ใบประวัติการคลอด', 85, finalY + 15)
  doc.setFontSize(18)
  ...
  doc.autoTable({
    startY: finalY + 50,
    head: [['วันที่', 'จำนวน(ตัว)','เวลา','รหัสน้ำเชื้อ/พ่อพันธุ์','ผู้ปฏิบัติ','หมายเหตุ']],
    columnStyles: {
      0: {cellWidth:25},
      1: {cellWidth: 25},
      2: {cellWidth: 20},
...
const queryDataExcel=async(id,index)=>{
  const res=await axios.get("https://aipcattle.herokuapp.com/delivery/history/" +props.UID+"/"+id)
   ...
   res.data.map(i=>{
    const newSet=[
      {
        value: i.date ,style: {border: borders,alignment: {wrapText: true, horizontal: 'left', vertical: 'top',}}
      },
      {
        value: i.count_calf,style: {border: borders,alignment: {wrapText: true, horizontal: 'left', vertical: 'top'}}
      },
      {
        value: i.time,style: {border: borders,alignment: {wrapText: true, horizontal: 'left', vertical: 'top'}}
      },
      {
        value: i.sire_id,style: {border: borders,alignment: {wrapText: true, horizontal: 'left', vertical: 'top'}}
      },
      {
        value: i.operator,style: {border: borders,alignment: {wrapText: true, horizontal: 'left', vertical: 'top'}}
      },
      {
        value: i.node,style: {border: borders,alignment: {wrapText: true, horizontal: 'left', vertical: 'top'}}
      },
...
const multiDataSet=[
  { 
    ...
    ySteps:0,
    columns: [
      {title: "ใบประวัติการคลอด"},//pixels width 
  ],
  data:[]
  },
  { 
  ...
  { 
    ...
    columns: [
      ...
      {title:"จำนวน(ตัว)", width: {wpx:80}, style: { border: borders, font: { bold: true }},},//char width 
      {title: "เวลา", width: {wpx:70}, style: { border: borders, font: { bold: true }},},
      {title: "รหัสน้ำเชื้อ/พ่อพันธุ์", width: {wpx:150}, style: { border: borders, font: { bold: true }},},
      {title: "ผู้ปฏิบัติ", width: {wpx:150}, style: { border: borders, font: { bold: true }},},
    ...
    return list.map((i,index)=>(
      ...
      <Grid container spacing={3}>  
        ...
        {dataExcel != null&&index===indexCheck ? 
                 ...
                 <ExcelSheet dataSet={dataExcel} name="ใบประวัติการคลอด"/>
             </ExcelFile>:null
            }   
