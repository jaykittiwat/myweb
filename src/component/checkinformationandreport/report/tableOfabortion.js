...
const  queryDataPDF= async(id)=>{
          ...
          const res=await axios.get("https://aipcattle.herokuapp.com/abortion/history/" +props.UID+"/"+id)
         ...
         dataSet.push([arrData[2],arrData[4],arrData[3]])
          })
...
const PDF=(data,profile,databrand)=>{
  ...
  doc.text('ใบประวัติการแท้ง', 85, finalY + 15)
  doc.setFontSize(18)
  ...
  doc.autoTable({
    startY: finalY + 50,
    head: [['วันที่', 'ท้องครั้งที่','หมายเหตุ']],
    columnStyles: {
      0: {cellWidth:25},
      1: {cellWidth: 40},
      2: {cellWidth: 40},
...
const multiDataSet=[
  ...
  { 
    ...
    ySteps:0,
    columns: [
      {title: "หมายเลขโค: "+res2.data[1].cattle_id||res2.data[1].birth_id , width: {wpx: 90},},
      {title: "วันที่เกิด: "+res2.data[1].birth_date, width: {wpx:90},},
  ...
  { 
    ...
    ySteps:0,
    columns: [
      {title: "น้ำหนักแรกเกิด: "+res2.data[1].birth_weight, width: {wpx: 100}, },
      {title: "พ่อพันธุ์: "+res2.data[1].sire_id, width: {wpx:100}, },
  ...
  { 
    ...
    ySteps:1,
    columns: [
      {title: "วันที่", width: {wpx:100}, style: { border: borders, font: { bold: true }},},
      {title:"ท้องครั้งที่", width: {wpx:100}, style: { border: borders, font: { bold: true }},},
