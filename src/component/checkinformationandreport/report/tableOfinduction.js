...
const  queryDataPDF= async(id)=>{
          ...
          const res=await axios.get("https://aipcattle.herokuapp.com/synchronize/history/" +props.UID+"/"+id)
         ...
         dataSet.push([arrData[1],arrData[2],arrData[3]])
          })
...
const PDF=(data,profile,databrand)=>{
  ...
  doc.text('ใบประวัติการเหนี่ยวนำ', 85, finalY + 15)
  doc.setFontSize(18)
  ...
  doc.autoTable({
    startY: finalY + 50,
    head: [['วันที่', 'ผู้ปฏิบัติ','โปรแกรมเหนี่ยวนำ']],
    columnStyles: {
      0: {cellWidth:45},
      1: {cellWidth: 65},
      2: {cellWidth: 65},
