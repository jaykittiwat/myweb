...
class Maintain extends Component {
    ...
    setChart() {
        if(this.state.form==="แสดงการเปรียบเทียบจำนวนโคในแต่ละปี"){
            axios.get("https://aipcattle.herokuapp.com/delivery/graph/" + this.props.UID + "/" + this.state.start + "-01-01/" + this.state.end + "-12-31/emtyp").then(res => {
            const setData = []
            ...
            axios.get("https://aipcattle.herokuapp.com/delivery/graph/" + this.props.UID + "/" + this.state.start + "-01-01/" + this.state.start + "-12-31/"+this.state.cattle).then(res => {
            const setData = []
            res.data.map((i, index) => {
                console.log(i);
                const sum = i.reduce(function (acc, val) { return acc + val; }, 0)
    ...
    componentDidUpdate(prevProps) {
        ...
        if (this.props.UID !== prevProps.UID) {
            axios.get("https://aipcattle.herokuapp.com/delivery/graph/" + this.props.UID + "/2020-01-01/2020-12-31/"+this.state.cattle).then(res => {
                const setData = []
                res.data.map((i, index) => {
                    const sum = i.reduce(function (acc, val) { return acc + val; }, 0)
                    if (sum !== 0) {
