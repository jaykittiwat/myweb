...
class Header extends Component {
  ...
  componentDidMount(){
    ...
    firebase.auth().onAuthStateChanged((user)=>{
      if(user){
        axios
        .get("http://localhost:38844/user/logIn/" + user.email).then(res=>{
          ...
          axios.get("http://localhost:38844/settingcolor/color/"+this.state.UID).then(res=>{
            this.setState({...this.state,color:res.data[1] });
          }).then(()=>{
            axios.get("http://localhost:38844/settingstrian/strian/"+this.state.UID).then(res=>{
              this.setState({...this.state,strian:res.data[1]});
            }).then(async ()=>{
              const bc=await axios.get("http://localhost:38844/settingbigcorral/bigcorral/"+this.state.UID)
              const cr=await axios.get("http://localhost:38844/settingcorral/corral/"+this.state.UID)
              const hn=await axios.get("http://localhost:38844/settingherd_num/herd_num/"+this.state.UID)
